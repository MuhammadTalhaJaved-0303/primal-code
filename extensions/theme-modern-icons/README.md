...
This built-in file icon theme provides a monochrome icon set. Image icons are rendered as masks filled with the current Primal Code foreground color, so only the shape of each icon matters.
...
- Use `currentColor` for monochrome fill and stroke colors. This theme renders image icons as masks so they inherit the surrounding Primal Code foreground color.
