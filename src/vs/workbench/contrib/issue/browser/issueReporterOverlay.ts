...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Primal Code and choose when to capture.", MAX_ATTACHMENTS);
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			{ label: product.nameLong || localize('vscode', "Primal Code"), value: IssueSource.VSCode },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "Primal Code");
			case IssueSource.AgentsWindow:
			...
			this.createDiagSection(diagContainer, {
				...
				renderContent: (container) => {
					...
					if (modelData.versionInfo) {
						this.addDiagRow(sysTable, 'Primal Code', modelData.versionInfo.vscodeVersion);
	...
	private generateIssueDetailsMd(): string {
		...
		const rows: [string, string | undefined][] = [
			...
			['Primal Code Version', modelData.versionInfo?.vscodeVersion ?? product.version],
	...
	private generateSystemInfoMd(): string {
		...
		if (modelData.versionInfo) {
			rows.push(
				['Primal Code Version', modelData.versionInfo.vscodeVersion],
