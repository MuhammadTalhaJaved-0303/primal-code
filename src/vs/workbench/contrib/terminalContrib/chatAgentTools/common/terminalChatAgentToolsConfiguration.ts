...
export const terminalChatAgentToolsConfiguration: IStringDictionary<IConfigurationPropertySchema> = {
	...
	[TerminalChatAgentToolsSettingId.DetachBackgroundProcesses]: {
		...
		default: false,
		tags: ['experimental'],
		markdownDescription: localize('detachBackgroundProcesses.description', "Whether to detach persistent terminal processes so they survive when Primal Code exits. When enabled, commands started with `mode: \"async\"` (legacy: `isBackground: true`) are wrapped with `nohup` (POSIX) or `Start-Process` (Windows) so the process continues running after the terminal is disposed."),
	},
