...
class WebviewFindAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	provideContent(): string {
		...
		content.push(localize('webview.importantDesc', "Some webviews intercept keyboard input before Primal Code's Find can use it. If Enter or Shift+Enter do not navigate matches, the webview may be handling those keys. Try clicking or tabbing into the webview content first to ensure the webview has focus, then reopen Find and try navigation again."));
		content.push('');
