			...
			async run(accessor: ServicesAccessor) {
				...
				if (connectionInfo) {
					...
					if (connectionInfo.link && connectionInfo.domain) {
						...
						notificationService.notify({
							...
							message:
								localize(
									...
									"You can now access this machine anywhere via the secure tunnel [{0}](command:{4}). To connect via a different machine, use the generated [{1}]({2}) link or use the [{6}]({7}) extension in the desktop or web. You can [configure](command:{3}) or [turn off](command:{5}) this access via the Primal Code Accounts menu.",
						...
						notificationService.notify({
							severity: Severity.Info,
							message: localize('progress.turnOn.final.noLink', "Remote Tunnel Access is enabled for {0}. You can [configure](command:{1}) or [turn off](command:{2}) this access via the Primal Code Accounts menu.", connectionInfo.tunnelName, RemoteTunnelCommandIds.configure, RemoteTunnelCommandIds.turnOff),
						});
					}
