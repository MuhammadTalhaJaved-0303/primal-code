...
function getUpdateHoverContent(updateState: StateType): MarkdownString {
	...
	switch (updateState) {
		case StateType.AvailableForDownload:
			hoverContent.appendMarkdown(localize('chat.modelPicker.downloadUpdateHover', "This model requires a newer version of Primal Code. [Download Update](command:update.downloadUpdate) to access it."));
			break;
		case StateType.Downloaded:
		case StateType.Ready:
			hoverContent.appendMarkdown(localize('chat.modelPicker.restartUpdateHover', "This model requires a newer version of Primal Code. [Restart to Update](command:update.restartToUpdate) to access it."));
			break;
		default:
			hoverContent.appendMarkdown(localize('chat.modelPicker.checkUpdateHover', "This model requires a newer version of Primal Code. [Update Primal Code](command:update.checkForUpdate) to access it."));
			break;
	}
	return hoverContent;
}
...
export function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		description = localize('chat.modelPicker.updateDescription', "Update Primal Code");
	} else {
