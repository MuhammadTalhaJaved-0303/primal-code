	...
	override async run(): Promise<any> {
		...
		if (this.extension.deprecationInfo) {
			...
			if (this.extension.deprecationInfo.extension) {
				...
				detail = localize('deprecated with alternate settings message', "This extension is deprecated as this functionality is now built-in to Primal Code.");
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload Primal Code to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.deprecationInfo) {
			if (this.extension.deprecationInfo.extension) {
				...
				this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize('deprecated with alternate settings tooltip', "This extension is deprecated as this functionality is now built-in to Primal Code. Configure these {0} to use this functionality.", link)) }, true);
			} else {
