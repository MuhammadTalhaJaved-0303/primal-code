	...
	private async showNewDefaultThemeNotification(previousSettingsId: string | undefined): Promise<void> {
		...
		const keepTheme = await new Promise(resolve => {
			this.notificationService.prompt(
				Severity.Info,
				nls.localize({ key: 'themeUpdatedNotification', comment: ['{0} is the name of the new default theme'] }, "Primal Code has a new default theme: '{0}'.", this.getColorTheme().label),
				[
					toAction({
