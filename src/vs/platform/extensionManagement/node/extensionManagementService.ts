	...
	async install(vsix: URI, options: InstallOptions = {}): Promise<ILocalExtension> {
		...
		try {
			...
			if (manifest.engines && manifest.engines.vscode && !isEngineValid(manifest.engines.vscode, this.productService.version, this.productService.date)) {
				throw new Error(nls.localize('incompatible', "Unable to install extension '{0}' as it is not compatible with Primal Code '{1}'.", extensionId, this.productService.version));
			}
			...
			if (existingExtension) {
				if (this.extensionKey.equals(new ExtensionKey(existingExtension.identifier, existingExtension.manifest.version))) {
					try {
						...
						throw new Error(nls.localize('restartCode', "Please restart Primal Code before reinstalling {0}.", this.manifest.displayName || this.manifest.name));
					}
				}
			}
