...
export const platformRootSchema = createSchema({
	...
	[AgentHostGlobalAutoApproveEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.globalAutoApproveEnabled.description', "Whether Primal Code's global auto-approve setting is enabled. When `true`, every tool call is auto-approved, equivalent to a session using Allow all."),
		default: false,
	}),
	...
	[AgentHostAutoReplyEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.autoReplyEnabled.description', "Whether Primal Code's auto-reply setting is enabled. When `true`, `ask_user` questions are auto-answered instead of blocking on the user, mirroring autopilot mode."),
		default: false,
	}),
