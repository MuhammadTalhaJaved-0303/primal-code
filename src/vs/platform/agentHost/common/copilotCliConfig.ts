...
export const copilotCliConfigSchema = createSchema({
	...
	[CopilotCliConfigKey.ToolSearchEnabled]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.toolSearchEnabled.description', "When enabled, Copilot SDK sessions defer MCP and non-core Primal Code tools behind a tool-search tool so the model discovers them on demand instead of loading every tool definition up front."),
		default: true,
	}),
		...
		additionalProperties: {
			...
			properties: {
				...
				availableTools: {
					...
					description: localize('agentHost.config.modelCapabilityOverrides.availableTools.description', "When set, only matching tools are available to sessions on this model. Patterns: bare tool names, `builtin:*` or `builtin:<name>` (Copilot runtime tools), `mcp:*` or `mcp:<name>` (MCP server tools), and `custom:*` or `custom:<name>` (every tool Primal Code registers with the SDK, including the agent host's own terminal tools); a bare `*` expands to all three sources. Applied when the session launches or resumes."),
				},
				excludedTools: {
					type: 'array',
